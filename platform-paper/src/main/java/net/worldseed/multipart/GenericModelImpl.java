...
public abstract class GenericModelImpl implements GenericModel {
    ...
    public void init(@NotNull World world, @NotNull Pos position) {
        init(world, position, 1);
    }

    public void init(@NotNull World world, @NotNull Pos position, float scale) {
        ...
        for (ModelBone modelBonePart : this.parts.values()) {
            ...
            modelBonePart.spawn(world, modelBonePart.calculatePosition()).join();
        }
    ...
    @Override
    public void sendPacketToViewers(@NotNull net.minecraft.network.protocol.Packet<?> packet) {
    ...
    @Override
    public void sendPacketsToViewers(@NotNull Collection<net.minecraft.network.protocol.Packet<?>> packets) {
        for (Player viewer : this.viewers) {
    ...
    @Override
    public void sendPacketsToViewers(@NotNull net.minecraft.network.protocol.Packet<?>... packets) {
        for (Player viewer : this.viewers) {
            for (net.minecraft.network.protocol.Packet<?> packet : packets) {
                ((CraftPlayer) viewer).getHandle().connection.send(packet);
            }
    ...
    @Override
    public void sendPacketToViewersAndSelf(@NotNull net.minecraft.network.protocol.Packet<?> packet) {
    ...
    public void addPartsAsPassengers(Player player) {
        getParts().forEach(part -> {
            try {
                player.addPassenger(part.getEntity().getBukkitEntity());
    ...
    public @NotNull Point relativeStart() {
        ...
        for (ModelBone bone : this.parts.values()) {
            for (var part : bone.getChildren()) {
                ...
                var absoluteStart = PaperConverter.nmsVecToPos(entity.getBoundingBox().getMinPosition());
    ...
    public @NotNull Point relativeEnd() {
        ...
        for (var bone : this.parts.values()) {
            for (var part : bone.getChildren()) {
                var entity = part.getEntity();
                var absoluteEnd = PaperConverter.nmsVecToPos(entity.getBoundingBox().getMaxPosition());
